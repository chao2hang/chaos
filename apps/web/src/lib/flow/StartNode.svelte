<script lang="ts">
	...
	import type { OrchestrationStartData } from '$lib/api';
	import { t } from '$lib/i18n.svelte';

	let { data, selected = false }: NodeProps = $props();
	const details = $derived(data as unknown as OrchestrationStartData);
</script>
...
<style>
	.anchor-node { position: relative; display: grid; grid-template-columns: auto minmax(0, 1fr); align-items: center; gap: .55rem; width: 10.5rem; min-height: 4.5rem; padding: .65rem .75rem; border: 1px solid var(--surface-inverse); border-radius: var(--radius-md); background: var(--surface-inverse); color: var(--ink-inverse); }
